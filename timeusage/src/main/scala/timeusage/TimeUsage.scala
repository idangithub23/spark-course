...
object TimeUsage extends TimeUsageInterface:
  ...
  implicit val timeUsageRowEncoder: Encoder[TimeUsageRow] = Encoders.kryo[TimeUsageRow]
  ...
  private def timeUsageByLifePeriod(): Unit =
  ...
  private def read(path: String): (List[String], DataFrame) =
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) =
    ...
    def filterPrime(str: String, list: List[String]): Boolean = list.exists(str.startsWith)

    def filterOther(str: String, list: List[String], excludeList: List[String]): Boolean = {
      val startsWithExcluded = excludeList.exists(str.startsWith)
      if (!startsWithExcluded) list.exists(str.startsWith)
      else false
    }

    val first = columnNames.filter(filterPrime(_, primaryNeeds)).map(col)
    val second = columnNames.filter(filterPrime(_, workingActivities)).map(col)
    val third = columnNames.filter(filterOther(_, otherActivities, otherActivitiesExcluded)).map(col)
  ...
  def timeUsageSummary(
    ...
    val sexProjection: Column = when(df(
      "tesex") === 1, "male").otherwise("female").as("sex")
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed
      .groupByKey(row => (row.working, row.sex, row.age))(Encoders.tuple(Encoders.STRING, Encoders.STRING, Encoders.STRING))
      .agg(
